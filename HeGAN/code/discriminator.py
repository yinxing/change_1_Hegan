...
class Discriminator():
    def __init__(self, n_node, n_relation, node_emd_init, type_emd, relation_emd_init):
        self.n_node = n_node
        self.n_relation = n_relation
        self.node_emd_init = node_emd_init
        self.type_emd = type_emd
        self.relation_emd_init = relation_emd_init
        self.emd_dim = node_emd_init.shape[1] + t_dimension
        ...
        self.t_emd_m = tf.cast(tf.constant(self.type_emd), tf.float32)

        self.node_embedding_matrix = tf.concat([self.node_embedding_matrix, self.t_emd_m], axis=1)
        #Mr

        # self.t_emd = tf.get_variable(name='type_embedding',
        #                              shape = self.type_emd.shape,
        #                              initializer=self.type_emd,
        #                              trainable=False)
